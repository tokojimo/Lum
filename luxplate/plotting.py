...
def plot_metric_points(metrics: pd.DataFrame, *, metric: str, y_scale: str = "linear",
    ...
    requested_log_scale = y_scale == "log"
    if requested_log_scale:
        positive = work[metric].gt(0)
        if positive.any():
            work = work.loc[positive].copy()
        else:
            # Matplotlib cannot lay out an empty logarithmic axis. Retain the finite
            # zero/negative observations on a linear axis instead of crashing the app.
            y_scale = "linear"
    ...
    axis.set_yscale(y_scale)
    if requested_log_scale and y_scale == "linear" and not work.empty:
        axis.text(.5, .98, "Log scale unavailable: no positive values",
