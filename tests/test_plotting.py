...
def test_mixed_panels_offer_log_luminescence_and_error_bars():
    ...
    assert [text.get_text() for text in figure.legends[0].get_texts()] == ["P0-lux", "Reporter-lux"]
    plt.close(figure)
...
def test_annotated_kinetics_excludes_blanks_and_deduplicates_legend_by_strain():
    strains = _publication_table()
    blank = strains.loc[strains["souche"].eq("P0-lux")].copy()
    blank["souche"] = "Blanc1"
    blank["type"] = "blanc"
    blank["sample_header"] = "blank-" + blank["sample_header"]
    data = pd.concat([strains, blank], ignore_index=True)
    metrics = run_kinetics(data).series_metrics

    figure = plot_kinetics(data, metrics)

    for axis in figure.axes:
        labels = axis.get_legend_handles_labels()[1]
        assert labels == ["P0-lux", "Reporter-lux"]
        assert "Blanc1" not in labels
    plt.close(figure)
