...
def test_package_figures_contains_raster_vector_formats_and_readme():
    ...
    assert rendered[0].tiff[:2] in {b"II", b"MM"}
    assert b"<svg" in rendered[0].svg
    assert rendered[0].pdf.startswith(b"%PDF")
    with ZipFile(BytesIO(payload)) as archive:
        assert set(archive.namelist()) == {
            "PNG/Milieu_1.png", "TIFF/Milieu_1.tiff", "SVG/Milieu_1.svg",
            "PDF/Milieu_1.pdf", "LISEZ_MOI.txt",
        }
    plt.close(figure)
