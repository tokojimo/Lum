...
COLONNES_MIN = ["temps_h", "souche", "type", "Lum_corr"]
...
def infer_output_dir(input_paths: Sequence[Path], output_dir: Path | None) -> Path:
    ...
    if len(input_paths) == 1:
        ...
        if parent_name.startswith("NORM_"):
            ...
            out = (input_path.parent / f"FIG_LUMINESCENCE_{cleaner_basename(input_path)}").resolve()
        out.mkdir(parents=True, exist_ok=True)
        return out
    ...
    out = (out_root / f"FIG_LUMINESCENCE_multi_{suffix}").resolve()
    out.mkdir(parents=True, exist_ok=True)
    return out
...
def prepare_growth_dataframe(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out = out.loc[out["Lum_corr"].notna()].copy()
...
def write_diagnostics(
    ...
    lines.append("- panel_LUMINESCENCE_par_milieu.png")
    lines.append("- panel_LUMINESCENCE_par_souche.png")

    (output_dir / "resume_courbes_LUMINESCENCE.txt").write_text(
        "\n".join(lines) + "\n",
        encoding="utf-8",
    )
...
def run_analysis(df: pd.DataFrame, output_dir: Path) -> None:
    ...
    summary_medium.to_csv(
        output_dir / "data_panel_LUMINESCENCE_par_milieu_summary.csv",
        index=False,
        encoding="utf-8-sig",
    )
    lines_medium.to_csv(
        output_dir / "data_panel_LUMINESCENCE_par_milieu_lines.csv",
        index=False,
        encoding="utf-8-sig",
    )
    ...
    plot_growth_panel(
        ...
        legend_title="Souche",
        output_path=output_dir / "panel_LUMINESCENCE_par_milieu.png",
        figure_title="Courbes de LUMINESCENCE - panneau par milieu",
    )
    ...
    summary_strain.to_csv(
        output_dir / "data_panel_LUMINESCENCE_par_souche_summary.csv",
        index=False,
        encoding="utf-8-sig",
    )
    lines_strain.to_csv(
        output_dir / "data_panel_LUMINESCENCE_par_souche_lines.csv",
        index=False,
        encoding="utf-8-sig",
    )
    ...
    plot_growth_panel(
        ...
        legend_title="Milieu",
        output_path=output_dir / "panel_LUMINESCENCE_par_souche.png",
        figure_title="Courbes de LUMINESCENCE - panneau par souche",
    )
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Générer les panneaux PNG des courbes de LUMINESCENCE (Lum_corr)."
    )
    parser.add_argument(
        "input_files",
        nargs="+",
...
def main() -> None:
    ...
    if df_growth.empty:
        raise ValueError(
            "Aucune donnée exploitable détectée pour les courbes de LUMINESCENCE. "
    ...
    print("=" * 80)
    print("COURBES DE LUMINESCENCE TERMINEES")
    print("=" * 80)
    ...
    print("- panel_LUMINESCENCE_par_milieu.png")
    print("- panel_LUMINESCENCE_par_souche.png")
    print("=" * 80)
