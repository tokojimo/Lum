...
- axe Y droite  : luminescence (Lum_corr, échelle log)
...
Style
-----
- lignes fines = séries / puits individuels
- ligne épaisse = résumé
...
def draw_subplot_dual_axis(
    ...
    line_group_col = "series_id" if "series_id" in lines_do.columns else "experience_id"

    sub_sum_do_all = summary_do.loc[summary_do.iloc[:, 0] == panel_value].copy()
    sub_lines_do_all = lines_do.loc[lines_do.iloc[:, 0] == panel_value].copy()
    sub_sum_lum_all = summary_lum.loc[summary_lum.iloc[:, 0] == panel_value].copy()
    sub_lines_lum_all = lines_lum.loc[lines_lum.iloc[:, 0] == panel_value].copy()

    lum_floor = None
    if ylim_right is not None:
        lum_floor = max(float(ylim_right[0]), np.finfo(float).tiny)
    else:
        lum_floor = np.finfo(float).tiny

    for compare_value in compare_order:
        color = color_map.get(compare_value, "#333333")

        # -------------------------
        # Croissance (axe gauche)
        # -------------------------
        sub_lines_do = sub_lines_do_all.loc[sub_lines_do_all[compare_col] == compare_value].copy()
        if not sub_lines_do.empty:
            for _, line_df in sub_lines_do.groupby(line_group_col, dropna=False):
        ...
        if not sub_sum_do.empty:
            ...
            ax_left.fill_between(x_do, y_do - sd_do, y_do + sd_do, color=color, alpha=0.14, linewidth=0, zorder=2)
            ax_left.plot(x_do, y_do, color=color, linewidth=1.8, linestyle="-", zorder=3)

        # -------------------------
        # Luminescence (axe droite)
        # -------------------------
        sub_lines_lum = sub_lines_lum_all.loc[sub_lines_lum_all[compare_col] == compare_value].copy()
        if not sub_lines_lum.empty:
            for _, line_df in sub_lines_lum.groupby(line_group_col, dropna=False):
                line_df = line_df.sort_values("temps_h")
                line_df = line_df.loc[line_df["line_value"] > 0].copy()
                if line_df.empty:
                    continue
                ax_right.plot(
                    line_df["temps_h"],
        ...
        if not sub_sum_lum.empty:
            sub_sum_lum = sub_sum_lum.loc[sub_sum_lum["mean_value"] > 0].copy()
            if not sub_sum_lum.empty:
                ...
                low_lum, high_lum = clip_log_band(y_lum, sd_lum, lum_floor)

                ax_right.fill_between(x_lum, low_lum, high_lum, color=color, alpha=0.08, linewidth=0, zorder=2)
                ax_right.plot(x_lum, y_lum, color=color, linewidth=1.8, linestyle=LUM_DASH, zorder=3)
    ...
    ax_right.set_ylabel("Luminescence (log)")
    ax_right.set_yscale("log")
    ax_right.yaxis.set_major_formatter(LogFormatterMathtext())
...
def plot_dual_panel_par_milieu(
    ...
    subtitle = (
        f"Solid lines = growth (left axis, {do_text}); dashed lines = luminescence (right axis, {lum_text}); "
        "thin lines indicate individual series."
    )
...
def write_diagnostics(
    ...
    lines.append(f"Axe Y droite (Lum, log) : {ylim_lum}")
    lines.append("")
    lines.append("Convention graphique :")
    lines.append("- trait plein = croissance")
    lines.append("- pointillés = luminescence")
...
def run_analysis(df: pd.DataFrame, output_dir: Path) -> None:
    ...
    lum_pos = lum_vals[(lum_vals.notna()) & (lum_vals > 0)]
    if lum_pos.empty:
        raise ValueError("Impossible de tracer Lum_corr en log : aucune valeur strictement positive.")
    global_lum_ylim = (float(lum_pos.min()), float(lum_pos.max()) * 1.05)
