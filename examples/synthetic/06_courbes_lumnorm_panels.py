...
Objectif
--------
Générer 2 panneaux PNG pour les courbes de croissance (Lum_norm) :
...
COLONNES_MIN = ["temps_h", "souche", "type", "Lum_norm"]
...
# Couleurs demandées pour les souches
STRAIN_COLORS = {
    "14.1Ac": "#1f77b4",  # bleu
...
def infer_output_dir(input_paths: Sequence[Path], output_dir: Path | None) -> Path:
    ...
    if len(input_paths) == 1:
        ...
        if parent_name.startswith("NORM_"):
            ...
            out = (input_path.parent / f"FIG_LUMINESCENCE_NORM_{cleaner_basename(input_path)}").resolve()
        out.mkdir(parents=True, exist_ok=True)
        return out
    ...
    out = (out_root / f"FIG_LUMINESCENCE_NORM_multi_{suffix}").resolve()
    out.mkdir(parents=True, exist_ok=True)
    return out
...
def prepare_growth_dataframe(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out = out.loc[out["Lum_norm"].notna()].copy()
...
def draw_subplot(
    ...
    sub_sum_all = summary.loc[summary.iloc[:, 0] == panel_value].copy()
    sub_lines_all = indiv_lines.loc[indiv_lines.iloc[:, 0] == panel_value].copy()
    ...
    ax.set_ylabel("LUM normalisé")
    formatter = ScalarFormatter(useMathText=True)
    formatter.set_scientific(True)
    formatter.set_powerlimits((0, 0))   # force la notation scientifique sur tous les axes
...
def plot_growth_panel(
    ...
    if panel_title_prefix == "Milieu" and n_panels == 3:
        nrows, ncols = 1, 3
    else:
...
def write_diagnostics(
    ...
    lines.append("- panel_LUMINESCENCE_NORM_par_milieu.png")
    lines.append("- panel_LUMINESCENCE_NORM_par_souche.png")

    (output_dir / "resume_courbes_LUMINESCENCE_NORM.txt").write_text(
        "\n".join(lines) + "\n",
        encoding="utf-8",
    )
...
def run_analysis(df: pd.DataFrame, output_dir: Path) -> None:
    ...
    summary_medium.to_csv(
        output_dir / "data_panel_LUMINESCENCE_NORM_par_milieu_summary.csv",
        index=False,
        encoding="utf-8-sig",
    )
    lines_medium.to_csv(
        output_dir / "data_panel_LUMINESCENCE_NORM_par_milieu_lines.csv",
        index=False,
        encoding="utf-8-sig",
    )
    ...
    plot_growth_panel(
        ...
        legend_title="Souche",
        output_path=output_dir / "panel_LUMINESCENCE_NORM_par_milieu.png",
        figure_title="Courbes de LUMINESCENCE_NORM - panneau par milieu",
    )
    ...
    summary_strain.to_csv(
        output_dir / "data_panel_LUMINESCENCE_NORM_par_souche_summary.csv",
        index=False,
        encoding="utf-8-sig",
    )
    lines_strain.to_csv(
        output_dir / "data_panel_LUMINESCENCE_NORM_par_souche_lines.csv",
        index=False,
        encoding="utf-8-sig",
    )
    ...
    plot_growth_panel(
        ...
        legend_title="Milieu",
        output_path=output_dir / "panel_LUMINESCENCE_NORM_par_souche.png",
        figure_title="Courbes de LUMINESCENCE_NORM - panneau par souche",
    )
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Générer les panneaux PNG des courbes de LUMINESCENCE_NORM (Lum_norm)."
    )
    parser.add_argument(
        "input_files",
        nargs="+",
...
def main() -> None:
    ...
    if df_growth.empty:
        raise ValueError(
            "Aucune donnée exploitable détectée pour les courbes de LUMINESCENCE_NORM. "
            "Vérifie la colonne 'souche', la colonne 'type' et la présence de Lum_norm."
        )

    run_analysis(df_growth, output_dir)

    print("=" * 80)
    print("COURBES DE LUMINESCENCE_NORM TERMINEES")
    print("=" * 80)
    ...
    print("- panel_LUMINESCENCE_NORM_par_milieu.png")
    print("- panel_LUMINESCENCE_NORM_par_souche.png")
    print("=" * 80)
