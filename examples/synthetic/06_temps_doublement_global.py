...
"""
06_temps_doublement_global.py

Objectif
--------
Générer UNE figure globale du temps de doublement mesuré à partir des courbes
de croissance (DO_corr), avec exactement le même style global que les scripts
du pic et de l'AUC.

Sorties
-------
- figure_temps_doublement_h.png
- figure_data_parametres_par_serie.csv
- figure_data_temps_doublement_h_points.csv
- diagnostic_conditions_detectees.txt
- resume_temps_doublement.txt
"""
...
def linear_fit_stats(x: np.ndarray, y: np.ndarray) -> tuple[float, float, float]:
    """
    Retourne slope, intercept, r2 pour y = slope*x + intercept
    """
    if len(x) < 2:
...
def estimate_doubling_time_from_growth(
    time_h: pd.Series,
    od: pd.Series,
    min_points_window: int = 3,
    min_r2: float = 0.80,
) -> tuple[float, float, float, float, int]:
    """
    Estime le temps de doublement à partir de la meilleure fenêtre exponentielle
    sur ln(DO_corr) vs temps.
    ...
    - n_points_used
    """
    valid = pd.DataFrame({"temps_h": time_h, "DO_corr": od}).dropna()
    valid = valid.loc[valid["DO_corr"] > 0].sort_values("temps_h").reset_index(drop=True)

    if len(valid) < min_points_window:
    ...
    for start in range(0, len(valid) - min_points_window + 1):
        for end in range(start + min_points_window, len(valid) + 1):
            xw = x[start:end]
            yw = y[start:end]

            slope, intercept, r2 = linear_fit_stats(xw, yw)
    ...
    if best is None:
        # fallback : meilleure pente positive même si R² faible
        for start in range(0, len(valid) - min_points_window + 1):
            for end in range(start + min_points_window, len(valid) + 1):
                xw = x[start:end]
                yw = y[start:end]
                ...
                score = (slope, -np.inf if not np.isfinite(r2) else r2, len(xw))
                if best is None or score > best["score"]:
                    best = {
                        "score": score,
                        "slope": slope,
    ...
    doubling_time_h = float(np.log(2.0) / best["slope"])
    return doubling_time_h, float(best["slope"]), float(best["r2"]), float(best["start_h"]), int(best["n_points"])
...
def extract_parameters_per_series(df: pd.DataFrame, category_col: str) -> pd.DataFrame:
    ...
    for keys, sub in work.groupby(group_cols, dropna=False, observed=True):
        ...
        doubling_h, slope_h_inv, r2, start_h, n_points_used = estimate_doubling_time_from_growth(
            sub["temps_h"],
            sub["DO_corr"],
        )
        row["temps_doublement_min"] = doubling_h * 60 if np.isfinite(doubling_h) else np.nan
        row["croissance_exp_slope_h_inv"] = slope_h_inv
        row["croissance_exp_r2"] = r2
        row["croissance_exp_start_h"] = start_h
        row["croissance_exp_n_points"] = n_points_used
...
def build_report(
    ...
    lines = []
    lines.append("ANALYSE TEMPS DE DOUBLEMENT\n")
    ...
    lines.append("")
    lines.append(f"temps_doublement_h : {'générée' if not points.empty else 'non générée'} (niveau des points = {point_level})")
    return "\n".join(lines) + "\n"
...
def run_analysis(df: pd.DataFrame, title: str, output_dir: Path) -> None:
    ...
    points, point_level = summary_points_for_metric(params, "temps_doublement_min")
    points.to_csv(output_dir / "figure_data_temps_doublement_min_points.csv", index=False, encoding="utf-8-sig")

    plot_metric_dots(
        points=points,
        metric="temps_doublement_min",
        ylabel="Temps de doublement (min)",
        title=f"{title} - Temps de doublement",
        basepath=output_dir / "figure_temps_doublement_min",
        category_order=category_order,
        strain_colors=strain_colors,
        point_level=point_level,
        positions=positions,
    ...
    (output_dir / "resume_temps_doublement.txt").write_text(report, encoding="utf-8")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Générer la figure globale du temps de doublement mesuré à partir de DO_corr."
    )
    parser.add_argument(
        "input_files",
        nargs="+",
...
def main() -> None:
    ...
    print("=" * 80)
    print("TEMPS DE DOUBLEMENT TERMINÉ")
    print("=" * 80)
    ...
    print("- figure_temps_doublement_h.png")
    print("=" * 80)
