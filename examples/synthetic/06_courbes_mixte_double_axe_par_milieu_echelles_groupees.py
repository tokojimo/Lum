...
- échelle de luminescence partagée par groupes de milieux comparables :
    * les deux milieux DMEM partagent une échelle
    * SCFM2-KPi et SCFM2-KPi (Spd) partagent une autre échelle
...
MEDIUM_ORDER = {
    # Expérience DMEM : les deux panneaux restent côte à côte.
    "2.5% SCFM2-KPi + 97.5% DMEM-SVF": 0,
    "2.5% SCFM2-KPi + 97.5% DMEM-SVF-KPi": 1,
    # Expérience SCFM2-KPi : les deux panneaux restent côte à côte.
    "SCFM2-KPi": 2,
    "SCFM2-KPi (Spd)": 3,
    # Ordre historique conservé pour les autres jeux de données.
    "LB": 10,
    "SCFM1": 11,
    "Old SCFM2": 12,
    "New SCFM2": 13,
...
# Groupes qui doivent partager la même échelle Y droite.
# Le mode par défaut est "grouped". Un milieu absent ne provoque pas d'erreur.
LUM_SCALE_GROUPS: dict[str, tuple[str, ...]] = {
    "DMEM": (
...
STRAIN_COLORS = {
    "14.1Ac": "#7f7f7f",
    "14.1Ac attB::P0-lux": "#1f77b4",       # bleu
    "14.1Ac attB::PspeD-lux": "#d62728",    # rouge
    "14.1Ac attB::PspeD2-1A-lux": "#2ca02c",# vert
    "14.1Ac attB::PspeD2-3B-lux": "#9467bd",# violet
    "14.1Ac attB::PspeE-lux": "#ff7f0e",    # orange
    # Anciennes variantes de noms encore acceptées.
    "14.1Ac attB::MiniCTXlux(P0-lux)": "#1f77b4",
    "14.1Ac attB::MiniCTXlux(PspeD-lux)": "#d62728",
    "14.1Ac attB::MiniCTXlux(PspeD2-1A-lux)": "#2ca02c",
    "14.1Ac attB::MiniCTXlux(PspeD2-3B-lux)": "#9467bd",
    "14.1Ac attB::MiniCTXlux(PspeE-lux)": "#ff7f0e",
}
...
def compute_linear_ylim_from_summary(
    summary: pd.DataFrame,
    panels: Sequence[str],
    panel_col: str = "medium",
    padding_fraction: float = 0.05,
) -> tuple[float, float] | None:
    """Calculer une limite linéaire couvrant moyenne ± SD pour les panneaux donnés.

    Si toutes les valeurs sont positives, l'axe commence à zéro. Les SD sont
    incluses afin que les barres d'erreur ne soient pas coupées.
    """
    if summary.empty or panel_col not in summary.columns:
        return None

    sub = summary.loc[summary[panel_col].astype(str).isin([str(x) for x in panels])].copy()
    if sub.empty:
        return None

    mean = pd.to_numeric(sub["mean_value"], errors="coerce").to_numpy(dtype=float)
    sd = pd.to_numeric(sub["sd_value"], errors="coerce").fillna(0.0).to_numpy(dtype=float)
    ...
    low_value = float(np.min(low[finite]))
    high_value = float(np.max(high[finite]))

    if low_value >= 0:
        lower = 0.0
        upper = high_value * (1.0 + padding_fraction)
        if upper <= 0:
            upper = 1.0
        return lower, upper

    span = high_value - low_value
    padding = span * padding_fraction if span > 0 else max(abs(high_value), 1.0) * padding_fraction
    return low_value - padding, high_value + padding


def build_lum_ylim_by_panel(
    summary_lum: pd.DataFrame,
    panel_order: Sequence[str],
    mode: str = "grouped",
) -> tuple[dict[str, tuple[float, float]], dict[str, str]]:
    """Construire les limites de luminescence par panneau.

    Modes :
    - grouped    : une échelle pour chaque groupe défini dans LUM_SCALE_GROUPS ;
                   les panneaux non répertoriés utilisent leur propre échelle.
    - global     : une seule échelle pour tous les panneaux.
    - individual : une échelle indépendante pour chaque panneau.

    Retourne également le nom du groupe attribué à chaque panneau pour le rapport.
    """
    if mode not in {"grouped", "global", "individual"}:
        raise ValueError(f"Mode d'échelle de luminescence inconnu : {mode}")

    panel_order = [str(x) for x in panel_order]
    ylim_by_panel: dict[str, tuple[float, float]] = {}
    group_by_panel: dict[str, str] = {}

    if mode == "global":
        ylim = compute_linear_ylim_from_summary(summary_lum, panel_order)
        if ylim is not None:
            for panel in panel_order:
                ylim_by_panel[panel] = ylim
                group_by_panel[panel] = "global"
        return ylim_by_panel, group_by_panel

    if mode == "individual":
        for panel in panel_order:
            ylim = compute_linear_ylim_from_summary(summary_lum, [panel])
            if ylim is not None:
                ylim_by_panel[panel] = ylim
                group_by_panel[panel] = panel
        return ylim_by_panel, group_by_panel

    assigned: set[str] = set()
    available = set(panel_order)

    for group_name, configured_panels in LUM_SCALE_GROUPS.items():
        present_panels = [panel for panel in configured_panels if panel in available]
        if not present_panels:
            continue

        ylim = compute_linear_ylim_from_summary(summary_lum, present_panels)
        if ylim is None:
            continue

        for panel in present_panels:
            ylim_by_panel[panel] = ylim
            group_by_panel[panel] = group_name
            assigned.add(panel)

    # Sécurité : tout milieu non défini dans LUM_SCALE_GROUPS garde une échelle propre.
    for panel in panel_order:
        if panel in assigned:
            continue
        ylim = compute_linear_ylim_from_summary(summary_lum, [panel])
        if ylim is not None:
            ylim_by_panel[panel] = ylim
            group_by_panel[panel] = f"individuel: {panel}"
...
def plot_dual_panel_par_milieu(
    ...
    figure_title: str,
    ylim_left: tuple[float, float] | None = None,
    ylim_right_by_panel: dict[str, tuple[float, float]] | None = None,
) -> None:
    ...
    for i, panel_value in enumerate(panel_order):
        ...
        current_ylim_right = (
            ylim_right_by_panel.get(str(panel_value))
            if ylim_right_by_panel is not None
            else None
        )
...
def write_diagnostics(
    ...
    lum_scale_mode: str,
    ylim_lum_by_panel: dict[str, tuple[float, float]],
    lum_group_by_panel: dict[str, str],
) -> None:
    lines = []
    ...
    lines.append(f"Mode des axes Y droits (Lum, linéaire) : {lum_scale_mode}")
    lines.append("Axes Y droits par milieu :")
    for medium in medium_order:
        group_name = lum_group_by_panel.get(str(medium), "non défini")
        ylim = ylim_lum_by_panel.get(str(medium))
        lines.append(f"- {medium} | groupe={group_name} | ylim={ylim}")
    lines.append("")
...
def run_analysis(df: pd.DataFrame, output_dir: Path, lum_scale_mode: str = "grouped") -> None:
    ...
    if summary_lum.empty:
        raise ValueError("Impossible de tracer Lum_corr : aucune valeur exploitable après agrégation.")

    lum_ylim_by_panel, lum_group_by_panel = build_lum_ylim_by_panel(
        summary_lum=summary_lum,
    ...
    plot_dual_panel_par_milieu(
        ...
        ylim_right_by_panel=lum_ylim_by_panel,
    )
    ...
    write_diagnostics(
        ...
        lum_scale_mode=lum_scale_mode,
        ylim_lum_by_panel=lum_ylim_by_panel,
        lum_group_by_panel=lum_group_by_panel,
    )
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--lum-scale-mode",
        choices=["grouped", "global", "individual"],
        default="grouped",
        help=(
            "Échelle de luminescence : grouped = une échelle commune par groupe "
            "de milieux (défaut), global = une échelle pour tous les panneaux, "
            "individual = une échelle propre à chaque panneau."
        ),
    )
...
def main() -> None:
    ...
    run_analysis(df_growth, output_dir, lum_scale_mode=args.lum_scale_mode)
    ...
    print(f"Mode d'échelle luminescence : {args.lum_scale_mode}")
