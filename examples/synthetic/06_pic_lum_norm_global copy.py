...
"""
06_pic_lum_norm_global.py

Objectif
--------
Générer UNE figure globale du pic de luminescence normalisée (Lum_norm_max),
avec exactement la même logique de tracé que dans le script d'origine :

- conditions issues directement de la colonne "souche"
- catégories du type :
    * LB [PAO1]
    * SCFM1 [PAO1]
    * Old SCFM2 [PAO1]
...
- regroupement par souche de base, avec les milieux côte à côte
- labels inclinés sous la figure
- axe Y en log pour Lum_norm_max
- exclusion des valeurs <= 0 uniquement pour ce graphe log
- en mode merge :
    * points = moyennes d'expériences biologiques
- si une seule expérience :
    * points = puits / séries techniques

Sorties
-------
- figure_Lum_norm_max.png
- figure_data_parametres_par_serie.csv
- figure_data_Lum_norm_max_points.csv
- diagnostic_conditions_detectees.txt
- resume_pic_lum_norm.txt
"""
...
# -----------------------------------------------------------------------------
# Tracé identique à l'original pour Lum_norm_max
# -----------------------------------------------------------------------------
...
def draw_dot_panel(
    ...
    use_log_scale = (metric == "Lum_norm_max")
...
def plot_metric_dots(
    ...
    fig_height_mm = 125 if metric == "Lum_norm_max" else 90
...
def build_report(
    ...
    lines = []
    lines.append("ANALYSE PIC LUMINESCENCE NORMALISÉE\n")
    ...
    lines.append("")
    lines.append(f"Lum_norm_max : {'générée' if not points.empty else 'non générée'} (niveau des points = {point_level})")
    return "\n".join(lines) + "\n"
...
def run_analysis(df: pd.DataFrame, title: str, output_dir: Path) -> None:
    ...
    points, point_level = summary_points_for_metric(params, "Lum_norm_max")
    points.to_csv(output_dir / "figure_data_Lum_norm_max_points.csv", index=False, encoding="utf-8-sig")

    plot_metric_dots(
        points=points,
        metric="Lum_norm_max",
        ylabel="Pic luminescence normalisée",
        title=f"{title} - Pic luminescence normalisée",
    ...
    (output_dir / "resume_pic_lum_norm.txt").write_text(report, encoding="utf-8")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Générer la figure globale du pic de luminescence normalisée (Lum_norm_max)."
    )
    parser.add_argument(
        "input_files",
        nargs="+",
...
def main() -> None:
    ...
    print("=" * 80)
    print("PIC LUMINESCENCE NORMALISÉE TERMINÉ")
    print("=" * 80)
